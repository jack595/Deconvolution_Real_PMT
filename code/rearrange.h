...
void rearrange(TString name, pars_waves pars){
	using namespace std ;
	bool debug=false;
	...
	str->Branch("waves","TH1D",&waveform_Rearrange);
	...
	if ( plot_ResultIntoPdf == true )
	{
		...
		plot_into_pdf(v1D_TH1D, pars.name_PdfDir+"rearange_waves/"+newname+"_rearanged_waves"+pars.name_RawWaveSelectOption+".pdf");
		TCanvas *c1=new TCanvas("c_startposition","c_waves",800,600);
		startposition->DrawCopy();
		c1->SaveAs(pars.name_PdfDir+"rearange_waves/"+newname+"_StartPointsHist"+pars.name_RawWaveSelectOption+".png");
	}
