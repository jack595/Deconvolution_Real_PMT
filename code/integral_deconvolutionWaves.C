...
void integral_deconvolutionWaves(const TString name)
{
    ...
    TFile* f_waves = new TFile(name+"_waves_tran_0.root","read");
    TTree* tree_waves = (TTree*) f_waves->Get("waves_trans");
    TH1D* h_waves = NULL;
    tree_waves->SetBranchAddress("dividedt", &h_waves);
    const int nDimension=512;
    int N = nDimension;
...
//      cout<<"iMax and iMin are : "<<i<< "  "<<tmpidx<<"  "<<AC[i]<<"  "<< h_waves->GetBinContent(i)<<endl;
        tmpv.at(tmpidx - imin)++;//project the y value to tmpv, so that to get eht baseline.
      }
...
//      if (HitCount == 0) return true;
    ...
    TH1D* h_Integral = new TH1D("h_Integral","h_Integral",1000,0,1000);
